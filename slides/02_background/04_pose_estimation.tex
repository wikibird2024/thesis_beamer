...
\begin{frame}
\frametitle{Mô hình nhận dạng đối tượng phân cấp}
\begin{figure}
    \centering
    \includegraphics[width=0.8\textwidth]{ob_ject.png}
\end{figure}
\end{frame}


\begin{frame}{Cấu trúc phân cấp và MediaPipe BlazePose}
\begin{columns}[T]
    %---------------- Column 1: Phân cấp ----------------%
    \column{0.48\textwidth}
    \textbf{Cấu trúc phân cấp:}
    \begin{itemize}
        \item \textbf{Primitives (Cấp 0)}: Thành phần cơ bản (điểm, đường).
        \item \textbf{Parts (Cấp 1)}: Tập hợp các Primitives tạo bộ phận.
        \item \textbf{Object (Cấp 2)}: Tập hợp Parts tạo đối tượng hoàn chỉnh.
    \end{itemize}

    %---------------- Column 2: BlazePose ----------------%
    \column{0.48\textwidth}
    \begin{block}{MediaPipe BlazePose}
        BlazePose tối ưu HPE 3D với:
        \begin{itemize}
            \item \textbf{Nodes:} Các module xử lý tín hiệu hình ảnh.
            \item \textbf{Edges:} Luồng dữ liệu giữa các module.
        \end{itemize}
