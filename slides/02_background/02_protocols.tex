...
% Slide 1: Tổng quan
\begin{frame}
\frametitle{Các Giao Thức Truyền Thông trong Hệ Thống Cảnh Báo Té Ngã}
\begin{center}
\Large Hệ thống phát hiện ngã với ba giao thức chính.
\end{center}

\begin{itemize}
\item \alert{SIP}: Truyền tải âm thanh/video cảnh báo thời gian thực
\item \alert{MQTT}: Vận chuyển dữ liệu cảm biến từ thiết bị IoT
\item \alert{JSON}: Định dạng cấu trúc dữ liệu trao đổi
\end{itemize}
...
\begin{columns}[T]
    ...
    \begin{itemize}
        \item \textcolor{blue}{Không gian}: Publisher và Subscriber không cần biết địa chỉ IP của nhau, giao tiếp qua broker
        \item \textcolor{blue}{Thời gian}: Không yêu cầu kết nối đồng thời, hỗ trợ retained messages và clean session
        \item \textcolor{blue}{Đồng bộ}: Truyền/nhận độc lập, giảm độ trễ và tăng hiệu suất
    \end{itemize}
    ...
    \begin{itemize}
        \item \textcolor{blue}{\textbf{QoS 0}}: "Fire and forget", nhanh nhưng có thể mất message – Dữ liệu cảm biến thường xuyên
        \item \textcolor{orange}{\textbf{QoS 1}}: Đảm bảo gửi ít nhất một lần, có thể trùng lặp – Cân bằng độ tin cậy và hiệu suất
        \item \textcolor{red}{\textbf{QoS 2}}: Đảm bảo gửi đúng một lần, tin cậy nhất nhưng chậm – Dữ liệu quan trọng
    \end{itemize}
...
\item Phổ biến trong IoT cho lưu trữ và truyền dữ liệu
\end{itemize}
\end{block}
...
\begin{itemize}
    \item \alert{MQTT}: Thu thập dữ liệu hiệu quả
