...
\begin{frame}{Tổng quan Kiến trúc Hệ thống Phát hiện Té ngã sẽ xây dựng} 
\begin{block}{Phân loại theo đầu vào}
\begin{itemize}
\item \textbf{Dựa trên cảm biến hình ảnh}: Xử lý hình ảnh cố định, yêu cầu máy chủ mạnh
...
\begin{columns}[c] % căn giữa theo chiều cao cột
    %------------- Cột chữ -------------%
    \column{0.5\textwidth}
    ...
    \column{0.5\textwidth}
    \begin{center}
        \includegraphics[width=0.7\linewidth]{images/esp32_c3.png}
    \end{center}
\end{columns}
\end{frame}
...
\begin{enumerate}
    ...
    \item Phát hiện té ngã sơ cấp và xử lý dữ liệu tại biên
    \item Truyền dữ liệu lên máy chủ (ưu tiên Wi-Fi, dự phòng 4G)
    \item Máy chủ xử lý tin và phát cảnh báo
    \item Kích hoạt các kênh cảnh báo (SMS/cuộc gọi...) tới các thiết bị cuối
\end{enumerate}
\end{frame}
